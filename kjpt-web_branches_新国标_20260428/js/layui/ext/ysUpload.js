/**
 * ysUpload
 * layui上传组件修改，由于不满足现在的业务，所以
 */
 
layui.define(['jquery', 'layer', 'laydate'], function (exports) {
    let $ = layui.jquery,
    layer = layui.layer,
    hint = layui.hint(),
    device = layui.device(),
    MOD_NAME = 'ysUpload',ELEM = 'layui-upload', THIS = 'layui-this', SHOW = 'layui-show', HIDE = 'layui-hide', DISABLED = 'layui-disabled'
    ...
    upload = {
        ...
        on: function(events,callback){
            return layui.onevent.call(this,MOD_NAME,call);
        }
    },
    //操作当前实例
    thisUpload = function() {
    ...
    Class.prototype.upload = function(files, type){
        ...
        //选择文件的回调（重传时传入 files 不执行 choose，避免多出一行）
        if(type === 'choose' || (options.auto && !files)){
        ...
        if(options.size > 0 && !(device.ie && device.ie < 10)){
            ...
            //判断是多个文件上传还是单个文件上传
            if(this.config.bindAction !="" && this.config.bindAction.length >=1){
                for(let key in that.files){
                    let fileObj = that.files[key];
                    if(fileObj.size >= 1024*options.size){
