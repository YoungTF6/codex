...
layui.define(['jquery', 'layer'], function(exports){
	...
	var obj = function(config){
		...
		this.createOption = function(){
				...
				var html = '';
				html +=	'<div class="layui-unselect layui-form-select layui-multipleM-select">';
				html += 		'<i class="layui-edge" style="pointer-events: none;"></i>';
				html +=				'<input '+verify+'name="'+inputName+'" type="text" readonly="" class="layui-input layui-unselect">';
				html +=			'<div class="layui-choose-box">';
				html +=				'<div class="layui-multiple-scroll">';
				html +=					'<div class="layui-input multiple"></div>';
				html +=				'</div>';
				html +=			'</div>';
				...
				for(var i=0;i<d.length;i++){
					...
					html +=			'<i class="layui-icon">&#xe605;</i><span>'+d[i][f.titleName].trim()+'</span>';
					html +=		'</div>';
					html +='</dd>';
				}
				html +=			'</dl>';
				html +=		'</div>';				
				$E.html(html);
				let container = document.querySelector(".layui-multiple-scroll");
		...
		this.setSelected = function(first){
			...
			if(items.length==0){
				...
				items.each(function(){
					...
					values.push(v);
					names.push(n);
					spans.push('<a href="javascript:;"><span lay-value="'+v+'">'+n+'</span><i class="layui-icon layui-icon-close"></i></a>');
					selected.push(item);
				});
			}
			...
			var h = $E.find('.multiple').height()+4;
  ...
  obj.prototype.render = function(){
		var o=this,c=o.config,f=c.field;var isBindClose=c.isBindClose;
		$E = $(c.elem);
		if($E.length==0){
		...
		var style = '.layui-selectm-close{font-family: "xm-iconfont" !important;color: #FFF;font-style: normal;margin-left: 8px;font-size: 12px;cursor: pointer;display: flex;}.layui-selectm-close::before{content:"\\e601";}.lay-ext-mulitsel .layui-form-select dl{width:100%;}.lay-ext-mulitsel .layui-form-select dl dd div{margin-top:0px!important;padding-left:0px;width:100%;display:flex;padding:5px 0;}.lay-ext-mulitsel .layui-form-select dl dd div span{padding-left:5px;padding-right:0;width:130px;white-space: break-spaces;}.lay-ext-mulitsel .layui-form-select dl dd.layui-this{background-color:#fff}.lay-ext-mulitsel .layui-input.multiple{display:flex;flex-wrap: nowrap;white-space: nowrap;border:none;margin-top:4px;}.lay-ext-mulitsel .layui-input.multiple a{align-items: baseline;display:flex;background:rgb(0, 150, 136);border-radius:2px;color:#fff;line-height:26px;height:26px;margin:2px 5px 2px 0;float:left;}.lay-ext-mulitsel .layui-input.multiple a span{padding: 0px 5px;}.lay-ext-mulitsel .layui-input.multiple a i{margin:0px 4px;font-size:14px;line-height:26px;} .lay-ext-mulitsel .layui-input.multiple a i:hover{background-color:#009E94;border-radius:2px;}.lay-ext-mulitsel .danger{border-color:#FF5722!important}.lay-ext-mulitsel .tips{pointer-events: none;position: absolute;left: 10px;top: 10px;color:#999999;}.layui-form-select .layui-choose-box{position: absolute;top: 0;bottom: 0px;left: 0px;right: 30px;overflow: auto hidden;}.layui-form-select .layui-choose-box .layui-multiple-scroll{overflow-y: hidden;min-height:36px;}.layui-form-select .layui-choose-box .layui-multiple-scroll::-webkit-scrollbar{display:none}.layui-multipleM-select{border: 1px solid #E6E6E6;}.layui-multipleM-select .layui-unselect{    border-style: none;}.layui-icon{}';
		...
		//展开/收起选项
		$E.on('click','.layui-multipleM-select',function(e){
			//隐藏其他实例显示的弹层
