...
layui.define(['laytpl', 'laypage', 'layer', 'form', 'util'], function(exports) {
    ...
    , table = {
        config: {
            ...
            indexName: 'LAY_TABLE_INDEX' //下标索引名
                ,
            disableName: 'myDisableStatus' //是否禁用
        } //全局配置项
        ,
        cache: {} //数据缓存
        ,
    ...
    Class.prototype.events = function() {
        ...
        var ELEM_GRID = 'layui-table-grid',
            ...
            gridExpand = function(hide) {
                ...
                } else if (elemCell.prop('scrollWidth') > Math.round(elemCell.outerWidth())) {
    ...
    table.checkStatus = function(id) {
        ...
        layui.each(data, function(i, item) {
            ...
            // 禁用行不参与全选判定（与表体 layTableCheckbox:not(:disabled) 一致）
            if (disableName && item[disableName]) {
                invalidNum++;
                return;
            }
        ...
        var selectableNum = data.length - invalidNum;
        return {
            data: arr //选中的数据
                ,
            isAll: selectableNum > 0 && nums === selectableNum //是否全选（仅统计可选行）
        };
    };
