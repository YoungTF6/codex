function getProjectPageUrl(holdYear, scoreLevelId, projectId, source, preview,projType,regionLevelId,projectType) {
    const standardKindId = localStorage.getItem("standardkind-id");
    const baseUrl = "../mod/declare";
    const commonParams = `?a=a&source=${source}&zh_cn=1&hold_year=${holdYear}&score_level_id=${scoreLevelId}&project_id=${projectId}&preview=${preview}&proj_type=${projType}&region_level_id=${regionLevelId}&project_type=${projectType}`;
    const paths = {
