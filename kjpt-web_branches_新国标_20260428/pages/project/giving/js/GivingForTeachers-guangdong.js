...
layui.config({
    ...
    function buildTeacherTreeData(flatList) {
        ...
        var result = teacherOrder.map(function (groupKey) {
            ...
            parentRow.teachVoList = groupedItems.map(function (row) {
                var childPeriod = row.period != null ? row.period : row.teachPeriod;
                var periodNum = Number(childPeriod);
                if (!isNaN(periodNum)) {
    ...
    function setDefault(datas) {
        ...
        for (let i = 0; i < datas.length; i++) {
            if(datas.score == null) {
                defaultScore = Math.floor(datas[i].period) * 2;
