let createBaseHtml = function () {
        ...
        <div class="layui-form layui-card-header layuiadmin-card-header-auto" lay-filter="searchForQualifiedPersonnel" style="background: #f5f5f5;">
            <div class="layui_mess">
                ...
                <div class="layui-inline" id="attendTimesDiv">
                    ...
                    <div class="layui-input-block" >
                        <input type="number" step="1" name="attendTimes" value=1 id="attendTimes_id" autocomplete="off" class="layui-input">
                    </div>
                </div>
    ...
    <form class="layui-form" style="display: none" id="fromForCreditStrategy">
        <div class="layui-form-item">
            ...
            <div class="layui-input-block">
                <input type="text" name="title" id="creditStrategyDate_ids" required  lay-verify="required" placeholder="请输入标题" autocomplete="off" class="layui-input" readonly="readonly"  style="cursor:pointer;">
            </div>
        </div>
...
layui.config({
    ...
    let initTeachTable = function (data) {
        ...
        layui.each(data || [], function (_, item) {
            ...
            //  参会时长不超过 80% 或者 考勤有交叉，禁止授分
            var disabledByAttend = disabledByDuration || disabledByCross;
            //  是否单位授分允许范围内
            var disabledByUnitLimit = !row.unitLimitAwardable;
            row.myDisableStatus = !!(disabledByScore || disabledByAttend || disabledByUnitLimit);
            ...
            row.myDisabledUnitLimit = !! disabledByUnitLimit;
            tableData.push(row);
        });
        table.render({
            elem: '#creditAnalysis',
    ...
    window.openAllChoose = function () {
        ...
        table.render({
            ...
            cols: [[
                ...
                {
                    field: 'meetingDuration', align: 'center', title: '参会时长', hide: !_isGuangdong, templet: (d) => {
                        if (d.downDuringInHours * 0.8 > d.meetingDuration) {
                            return `<span style='color:red;'> ${d.meetingDuration} </span>`;
                        } else {
                            return d.meetingDuration;
                ...
                { align: 'center', title: '考勤信息', templet: attendanceRecordAwardPointsCommon.createTopTableToolBar5() },
                {
                    title: "考勤时间是否与其他项目重叠", align: 'center', hide: !_isGuangdong, width: 220, templet: (d) => {
                        if (d.attendanceCross) {
                            return `<span style='color:red;'>是</span>`;
                        } else {
                            return "否";
    ...
    table.on('toolbar(creditAnalysisFilter)', function (obj) {
        ...
        if (obj.event === 'exportToScorePerson') {
            ...
            if (cmeStandardKindId != StandardKind.GUANG_DONG) {
                ...
                origCols = origCols.filter(function (c) {
                    return c !== 'meetingDuration' && c !== 'attendanceCross';
                });
            } else if (attendanceRecordAwardPointsCommon.hideGuangdongExtraColumns()) {
                delete transCols.meetingDuration;
                delete transCols.attendanceCross;
                origCols = origCols.filter(function (c) {
                    return c !== 'meetingDuration' && c !== 'attendanceCross';
    ...
    layer.ready(function () {
        ...
        let configNames = [
            ...
            'min_value_when_find_by_attend_times',
        ...
        $("#attendTimes_id").val(minTimes).on("input", function () {
            ...
            if ($this.val() < parseInt(minTimes)) {
                timeout = setTimeout(() => {
                    $this.val("");
                    layer.msg("该项目的考勤次数不能低于" + minTimes + "次", { icon: 2 });
                }, 400);
            }
        });
