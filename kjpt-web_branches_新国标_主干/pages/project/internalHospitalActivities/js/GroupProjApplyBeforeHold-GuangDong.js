layui.config({
    ...
    applyBeforeCommon: '/services/project/applyBefore/applyBeforeCommon'
});
// 全局配置及引入相关的模块
layui.use(['table', 'form', 'laydate', 'element', 'upload', 'opTable', 'treeSelect', 'tool', 'selectInput', 'applyBeforeCommon'], function () {
    var table = layui.table
        ...
        , infoIndex = null
        , uploadInst = null
        , applyBeforeCommon = layui.applyBeforeCommon;
        ;
    ...
    let initTeacherCardLayout = function () {
        // 结构调整只做一次：医通卡号/证件号紧接"是否主讲人授分"，授课教师移到第二行
        if (!$("#teacherRowTwo #teacherNameBlock").length) {
            $("#personNoBlock").insertBefore("#teachNoBlock");
            $("#teacherNameBlock").prependTo("#teacherRowTwo");
        }
        $("#personNoBlock").show();
    ...
    let showApplyFiles = function () {
        const hasUploadConfig = fileUpObj.some(function (item) {
            return item.fileName && item.fileType !== undefined && item.fileType !== null;
        }) || $('#filesDiv .meeting-notice').length > 0;
        $('#filesDiv').css("display", hasUploadConfig ? "block" : "none");
    };
    ...
    window.loaderTable = function () {
        table.render({
            ...
            cols: [[
                { align: 'center', minWidth: 200, title: '操作',templet: function (d) {
                    return applyBeforeCommon.generateTableBarGuandDongUnit(d);
                } },
    ...
    window.initTable = function (toolbarId, isHidenTool) {
        ...
        opTable = layui.opTable.render({
            ...
            , cols: [[
                ...
                { field: 'period', title: '学时', align: "center", width: 60 },
            ]],
            toolbar: toolbarId,
            cellMinWidth: 170,
            // height: 'full-450',
            ...
            $("#needScore").val(String(!(obj.data.needScore === false || obj.data.needScore === "false")));
                        ...
                        teacherWindowIndex = openTeacherWindow('修改教师');
            ...
            , openTable: function (itemData) {
                ...
                return {
                    ...
                    , cols: [[
                        ...
                        { field: 'educationName', title: '最高学历', align: "center" },
    ...
    let openTeacherWindow = function (title='修改教师') {
        let tempIndex = layer.open({
            type: 1,
            title: title,
            content: $('#addTeacher'),
            skin: '',
    ...
    table.on('tool(applyBeforeHold)', function (obj) {
        ...
        if (obj.event === 'apply' || obj.event === 'applyAgain' || obj.event === 'update' || obj.event === 'addAndEdit') {
            ...
            form.render();
            // 加载课程列表（广东套：填写时不显示「添加课程」「模版导入课程」「下载课程模版」三个按钮，与查看模式一致）
            initTable('', false);
    ...
    table.on('tool(cmeProjPosDownList)', function (obj) {
        ...
        if (obj.event === 'edit') {
            ...
            initTeacherCardLayout();
            teacherWindowIndex = openTeacherWindow('添加教师');
    ...
    form.on('submit(notPass)', function (data) {
        ...
        layer.prompt({
            ...
            success: function (layero) {
                $(layero).find('.layui-layer-content').prepend('<div style="color:#FF5722;font-size:12px;line-height:20px;margin-bottom:8px;">审核不通过后无法再对项目进行更改、删除</div>');
            }
    ...
    let getTeacherMatchWarnMsg = function () {
        return "未匹配到相关人员，请手动输入或检查证件号码/医通卡号是否有误";
    }
    ...
    // 检查个人在指定课程或周期下是否已打卡
    let getIsAttended = function () {
        if (!projId || projId === "") {
            return;
        }
        let url = huayi_projectscore_url + 'cmeGroupprojAttendance/getHasAttendDownByProjId';
        commonAjax("get", url, { groupId: projId },
            function () { layer.msg("检查打卡状态失败"); },
            function (res) {
                if (res.status == 200) {
                    attendedDownId = res.data;
                    // 初始化表格
                    loaderTable();
                }
            }
