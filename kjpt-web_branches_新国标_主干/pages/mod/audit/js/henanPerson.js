layui.config({
    ...
    function getSifter() {
        return {
            ...
            'reportState': null,
            'lastState': null,
            'includeLastStateList': null,
            'includeFlowChainStateList': null,
            'category': null,
            'unitTypes': null,
            'titleTypes': null,
            'comPersonId': _pid
    ...
    function downpdf(comPersonIds, cp) {
        let tempUnitId = allData.length > 0 ? allData[0].unitId : _unitId;
        if (comPersonIds) {
            // uuid: 37 * 1000
            ...
            let tempItem = allData[0];
            if(tempItem.reportState && tempItem.reportState != 92 && tempItem.lastState != 1){
                lat.failMsg('您的年度审核已上报，待主管部门审核后，您可下载证书');
                return;
            } else {
                lat.failMsg('您的年度审核未上报，待单位上报且主管部门审核后，您可下载证书');
                return;
            }
