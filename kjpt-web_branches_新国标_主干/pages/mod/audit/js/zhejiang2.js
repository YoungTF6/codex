layui.config({
    ...
    let table = layui.table, layer = layui.layer, form = layui.form, laydate = layui.laydate, dropdown = layui.dropdown, element = layui.element, transfer = layui.transfer, $ = layui.jquery, xmSelect = layui.xmSelect, eleTree = layui.eleTree, lat = layui.lat;
    let gLayerIndex = -1, gDeptIds;
    layer.ready(function () {
        $('#tree_box div:first-child span').text(_isGov ? '选择单位' : '选择科室');
        lat.verifyTenancy(_isZhejiang);
        watermark.set('浙江省');
    });
    ...
    $(function () {
        if (_isPerson) {
            $('.person_hide').hide();
            $('input[name=personNo]').val(gpno).attr('disabled', 'true').toggleClass('layui-disabled');
            $('input[name=personName]').val(gpname).attr('disabled', 'true').toggleClass('layui-disabled');
            $('input[name=certId]').val(gpcid).attr('disabled', 'true').toggleClass('layui-disabled');
        } else {
            _tsf.loadTreeData();
            _tsf.renderTransfer();
        }
        lat.renderPersonState('select[name=personStateId]');
        lat.renderYearBox('#year_checkbox_container', 2026, 11);
    ...
    function getPersonSifter(source) {
        ...
        //
        let personNo = '', personName = '', certId = '', personStateId = null;
        if (fromQuery) {
            ...
            if(!personStateId) personStateId = null;
        }
        return {
            'scoreTypeVal': 7,
    ...
    function exportPdf() {
        ...
        if (_isPerson) {
            ...
            pids = transferSelList.map(i => i.value).join(',');
            if (!pids) {
                lat.failMsg('请选择人员');
                return false;
            }
            if (pids.length > 37000) {
                lat.failMsg('超过每次导出1000人数限制');
                return false;
            }
        }
        let visit = huayi_sjwh_url + 'audit/pdf/down'; // zhejiang-2 comPersonIds
        let params = {
            'pdfType': 2,
            'standardKindId': _standardKindId,
            'comPersonIds': pids,
        ...
        lat.downloadWithProgress(visit, params, '继续医学教育学分验证证明.pdf');
    }
});