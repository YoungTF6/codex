    ...
    const _action = {
        ...
        submit2: function (params, toNext) {
            if (gPrincipalCeilMsg) {
                ...
                if (typeof _dbase.guard?.beforeSubmitPrincipal === 'function') {
                    let guardMsg = _dbase.guard.beforeSubmitPrincipal(params, {
                        scoreLevelId: _dbase.info.scoreLevelId,
                        titleTreeData: gTitleTreeData
                    });
                    if (guardMsg) {
                        _dbase.enableSubmit();
                        lat.failMsg(guardMsg);
                        return false;
                    }
                }
                params.userCreate = _userId;
        ...
        submit3: function (courseArr, toNext) {
            ...
            if (typeof _dbase.guard?.beforeSubmitCourse === 'function') {
                msg = _dbase.guard.beforeSubmitCourse(courseArr, {
                    scoreLevelId: _dbase.info.scoreLevelId
                });
                if (msg) {
                    lat.failMsg(msg);
            ...
            if (!_guard.validatePrincipalCourseFeature((_echo.tchr?.courseTrim?.(courseArr) || courseArr))) {
                return false;
            } else {
        ...
        submit4: function (params, flag) {
            ...
            if (typeof _dbase.guard?.beforeSubmitCycle === 'function') {
                let guardMsg = _dbase.guard.beforeSubmitCycle(params, {
                    scoreLevelId: _dbase.info.scoreLevelId,
                    cycleArr: gCycleArr,
                    courseArr: gCourseArr,
                    calc: _dbase.calc,
                    projectDetail: gProjectDetail
                });
                if (guardMsg) {
                    _dbase.enableSubmit();
                    lat.failMsg(guardMsg);
    ...
    const _render = {
        ...
        renderTitleSel: function () {
            getTitleTreeData().then(response => {
                ...
                if (typeof _dbase.guard?.filterPrincipalTitleTree === 'function') {
                    d = _dbase.guard.filterPrincipalTitleTree(d, {
                        scoreLevelId: _dbase.info.scoreLevelId
                    });
                }
