    ...
    layui.config({
        ...
        function formatSimilarCheckDate(value) {
            if (!value) {
                return '';
            }
        ...
        function getCurrentStudyDateForSimilarCheck(data) {
            if (data && data.studyDate) {
                return formatSimilarCheckDate(data.studyDate);
            }
            if ($("#activityDate").length && $("#activityDate").val()) {
                return formatSimilarCheckDate($("#activityDate").val());
            }
            var year = window.getCmeYear ? window.getCmeYear() : '';
            return year ? year + '-01-01' : '';
        }

        function getSimilarProjectNameCheckData(data) {
            data = data || {};
            var projectName = data.singleProjName || $("input[name='singleProjName']:visible").val() || $("input[name='singleProjName']").val();
            var personId = data.comPersonId;
            if (!personId && typeof window.getSingleScorePersonId == 'function') {
                personId = window.getSingleScorePersonId();
            }
            var personIds = data.personIds || data.comPersonIds || [];
            if ((!personIds || personIds.length === 0) && typeof window.getSingleScorePersonIds == 'function') {
                personIds = window.getSingleScorePersonIds() || [];
            }
            return {
                singleProjName: projectName ? $.trim(projectName) : '',
                comPersonId: personId || '',
                personIds: personIds,
                studyDate: getCurrentStudyDateForSimilarCheck(data),
                scoreId: data.scoreId || (typeof window.getSingleScoreId == 'function' ? window.getSingleScoreId() : '')
            };
        }

        function getSimilarProjectNameConfirmKey(data) {
            if (!data || !data.singleProjName || (!data.comPersonId && (!data.personIds || data.personIds.length === 0)) || !data.studyDate) {
                return '';
            }
            var personKey = data.comPersonId || data.personIds.join(',');
            return personKey + '|' + data.studyDate.substring(0, 4) + '|' + data.singleProjName;
        }

        // 统一校验项目名称与同身份证同年度历史记录是否相似，命中后由用户确认是否继续录入。
        window.checkSimilarProjectName = function(data, confirmedCallback, cancelCallback) {
            var checkData = getSimilarProjectNameCheckData(data);
            var confirmKey = getSimilarProjectNameConfirmKey(checkData);
            if (!confirmKey) {
            ...
            $.ajax({
                type: 'post',
                url: huayi_projectscore_url + 'cmeDeptSingleprojScore/checkSimilarProjectName',
                data: JSON.stringify(checkData),
                contentType: 'application/json;charset=UTF-8',
                dataType: 'json',
                success: function(res) {
                    var result = res && res.data ? res.data : {};
                    if (!result.hit) {
                        if (typeof confirmedCallback == 'function') confirmedCallback();
                        return;
                    }
                    layer.confirm(result.message, {
                        title: result.same ? '强提示' : '提示',
                        btn: ['继续录入', '取消']
                    }, function(index) {
                        layer.close(index);
                        if (typeof confirmedCallback == 'function') confirmedCallback();
        ...
        $(document).on('blur', "input[name='singleProjName']", function() {
            window.checkSimilarProjectName({ singleProjName: $(this).val() });
        });

        // 批量录入场景只汇总人员ID并触发一次统一提示，不暴露具体人员或历史记录明细。
        window.checkSimilarProjectNameForScores = function(scores, confirmedCallback, cancelCallback) {
            ...
            scores.forEach(function(score) {
                if (score.comPersonId && personIds.indexOf(score.comPersonId) === -1) {
                    personIds.push(score.comPersonId);
                }
                var scorePersonIds = score.personIds || score.comPersonIds || [];
                scorePersonIds.forEach(function(personId) {
                    if (personId && personIds.indexOf(personId) === -1) {
                        personIds.push(personId);
                    }
                });
            });
            window.checkSimilarProjectName({
                singleProjName: firstScore.singleProjName,
                studyDate: firstScore.studyDate,
                personIds: personIds,
                scoreId: firstScore.scoreId
            }, confirmedCallback, cancelCallback);
        };
        ...
        window.loadZjScoreForm = function(formId, data, type){
            ...
            .then(html => {
                ...
                switch(scoreLevel){
                    ...
                    case 'c2967846-318f-4e88-b323-2853c2d63fa9':
                    case '78a943c6-84e6-45ab-8e1f-81a446d39bea':
                    case '9f4022dc-d341-44b9-b038-a08ec6f0c4f6':
                    case '8bf82417-9da7-4925-9324-75fd9a560169':
                    case '3d6a4d5b-5e09-4c90-a369-700016acfcd7':
