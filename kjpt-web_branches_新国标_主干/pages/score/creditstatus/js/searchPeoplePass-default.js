            ...
            <div class="layui-form layui-card-header layuiadmin-card-header-auto" style="background-color: #f5f5f5;">
                <div class="layui_mess">
                    <div class="layui_display">
                        ...
                        <div class="layui-inline layui-col-md3 changchun_member_show" style="display:none;">
...
layui.use(['table', 'form', 'element', 'laydate', 'tool', 'treeSelect', 'xmSelect'], function () {
    var $ = layui.jquery
        , table = layui.table
    ...
    var unitId = localStorage.getItem("unit-id")
        ...
        , pageSize
        , xmSelectPersonState
        , isChangchunScope = false
        ;
    const CHANGCHUN_CITY_HEALTH_UNIT_ID = '082100';
    const IS_ZHEJIANG =  StandardKind.ZHE_JIANG == cmeStandardKindId;
    const IS_JILIN = StandardKind.JILIN == cmeStandardKindId;
    const IS_GUANGXI = StandardKind.GUANG_XI == cmeStandardKindId;
    // note 删除测试数据
    // if (unitId == '' || unitId == undefined) unitId = '090103';
    // if (cmeStandardKindId == '' || cmeStandardKindId == undefined) cmeStandardKindId = '6427ddba-c02f-4229-bd73-49fc1c5d21f6';
    ...
    // 组装表头模板
    let tempArr = [];
    function getDynamicScoreCols() {
        let publicScoreCols = [];
        let otherScoreCols = [];
        let appendCol = function (title, factorId) {
            let scoreCol = { filed: "tt", minWidth: 100, title: title, align: 'center', factorId: factorId, templet: setTemplate };
            if (IS_JILIN && title && title.indexOf('公需') > -1) {
                publicScoreCols.push(scoreCol);
            } else {
                otherScoreCols.push(scoreCol);
            }
        };

        for (const obj of scoreKindCols) {
            appendCol(obj["kindName"], 'Z' + obj["kindId"]);
        }
        for (const obj of scoreLevelCols) {
            appendCol(obj["scoreLevelName"], 'F' + obj["scoreLevelId"]);
        }

        return {
            publicScoreCols: publicScoreCols,
            otherScoreCols: otherScoreCols
        };
    }

    function appendScoreCols(fieldArr, scoreCols) {
        scoreCols.forEach(function (scoreCol) {
            fieldArr.push(scoreCol);
        });
    }
    ...
    let getColsData = function () {
        ...
        let dynamicScoreCols = getDynamicScoreCols();
        appendScoreCols(fieldArr, dynamicScoreCols.publicScoreCols);
        if($('#year').val() >= '2025' && !IS_ZHEJIANG && !IS_GUANGXI){
            fieldArr.push({
        ...
        if(IS_ZHEJIANG){
            fieldArr.push({
                filed: "scoreKindMap.公需课学分", title: "公需科目学分", align: 'center', minWidth: 80,
                templet: function (d) { if (d["scoreKindMap"].公需课学分 == null) return 0; return d["scoreKindMap"].公需课学分 }
            });
            // fieldArr.push({
            //     filed: "scoreKindMap.专业课学分", title: "专业课学分", align: 'center', minWidth: 80,
            //     templet: function (d) { 
            //             if (d["totalPeriod"] == null ) return 0;
            //             if (d["scoreKindMap"].公需课学分 == null ) return d["totalPeriod"];
            //             let res = d["totalPeriod"] - d["scoreKindMap"].公需课学分;
            //             return res < 0 ? 0 : res ;
            //         }
            // });
        }
        appendScoreCols(fieldArr, dynamicScoreCols.otherScoreCols);
    ...
    /** 控制会员状态筛选显示，仅在吉林082100范围内展示 */
    function toggleMemberStatusFilter(visible) {
        isChangchunScope = !!visible;
        if (isChangchunScope) {
            $(".changchun_member_show").show();
        } else {
            $(".changchun_member_show").hide();
            $('#memberStatusSelect').val('');
        }
        form.render('select');
    }

    /** 递归判断目标单位是否位于指定单位树中 */
    function containUnitId(unitTree, targetUnitId) {
        if (!unitTree || !targetUnitId) {
            return false;
        }
        for (var i = 0; i < unitTree.length; i++) {
            var node = unitTree[i] || {};
            var nodeId = node.id || node.unit_id || node.unitId || node.value;
            if (nodeId == targetUnitId) {
                return true;
            }
            var children = node.children || node.child || [];
            if (children.length > 0 && containUnitId(children, targetUnitId)) {
                return true;
            }
        }
        return false;
    ...
    /** 初始化082100范围判定，非范围单位隐藏会员状态筛选 */
    function initMemberStatusScope() {
        if (!IS_JILIN) {
            toggleMemberStatusFilter(false);
            return;
        }
        menuUnitId = tool.isEmpty(localStorage.getItem("menu-unit-id")) ? unitId : localStorage.getItem("menu-unit-id");
        if (menuUnitId == CHANGCHUN_CITY_HEALTH_UNIT_ID) {
            toggleMemberStatusFilter(true);
            return;
        }
        $.ajax({
            type: "get",
            url: huayi_personorg_url + 'comunit/childrenAllTreeRedis?unitId=' + CHANGCHUN_CITY_HEALTH_UNIT_ID,
            dataType: 'json',
            success: function (res) {
                var treeData = res ? res : [];
                toggleMemberStatusFilter(containUnitId(treeData, menuUnitId));
            },
            error: function () {
                toggleMemberStatusFilter(false);
            }
    ...
    window.loaderTable = function () {
        ...
        var memberStatus = (IS_JILIN && isChangchunScope) ? $('#memberStatusSelect').val() : '';
                ...
                xmSelectPersonState = xmSelect.render({
                    ...
                    data: function () {
                        ...
                        for (var i = 0; i < data.data.records.length; i++) {
                            if( IS_ZHEJIANG && data.data.records[i].person_state_name == '产假'){
                                continue;
                            }
    ...
    form.on('select(layui-select-year)', function(data){
        if(data.value < '2025' || (IS_GUANGXI && data.value <= '2025')){
            scoreLevelCols = [];
            getScoreKind();
        }else{
            scoreKindCols = [];
