...
layui.use([ 'form', 'layer', 'jquery' ], () => {
    ...
    window.Login = {
        ...
        success: () => {
            ...
            Login.skipTwoFactorVerification(xhr.getResponseHeader('Authorization'));
        ...
        skipTwoFactorVerification: (token) => {
            $.ajax({
                type: 'POST',
                url: huayi_permission_url + 'feign/setTwoFactorVerificationFlag',
                async: true,
                data: {
                    token: token,
                    twoFactorVerificationFlag: true
                },
                ...
                success: (data, textStatus, xhr) => {
                    if (200 == data.status) {
                        console.log('跳过二次手机验证');
                    } else {
                        console.log('跳过二次手机验证失败');
                    }
                },
                error: (XMLHttpRequest, textStatus, errorThrown) => {
                    msg(layer, '跳过二次手机验证失败', 7, 1500);
                }
            });
        },
