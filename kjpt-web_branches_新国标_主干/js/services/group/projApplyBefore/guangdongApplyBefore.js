...
layui.use(['table', 'form', 'laydate', 'element', 'upload', 'opTable', 'treeSelect', 'tool', 'selectInput'], function () {
        ...
        table.render({
            ...
            cols: [[
                { align: 'center', minWidth: 200, title: '操作',templet: function (d) {
                    if(attendedDownId.indexOf(d.downId) >= 0 || d.keyId != null){
                        return '<a class="layui-btn layui-btn-xs layui-btn-normal btn-normal-row" lay-event="detail">查看</a>';
                    }
                    return '<a class="layui-btn layui-btn-xs" lay-event="addAndEdit" style="color: #27b1a2;background-color: transparent;">填写</a>';
    ...
    function initCertVerifyInfo(needScore) {
        if (isGuangdongTeacherCard) {
            ...
            $("#teachNo").attr("lay-verify", "rightCertId");
        } else {
            $("[data-id='spanOnCert']").hide();
            $("#teachNo").removeAttr("lay-verify");
        }
